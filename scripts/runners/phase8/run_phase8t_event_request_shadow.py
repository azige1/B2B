...
REPORTS_DIR = os.path.join(PROJECT_ROOT, "reports", "phase8_event_request_shadow")
MODEL_DIR = os.path.join(PROJECT_ROOT, "models_phase8_event_request_shadow")
TIMING_LOG = os.path.join(REPORTS_DIR, "phase8_event_request_shadow_timing_log.csv")

ANCHORS = [
    x.strip()
    for x in os.environ.get("PHASE8_EVENT_REQ_ANCHORS", "2025-09-01,2025-10-01,2025-11-01,2025-12-01").split(",")
    if x.strip()
]
SEED = "2028"
TREE_BACKEND = "lightgbm"
VAL_MODE = "single_anchor"
FORCE_REBUILD = os.environ.get("PHASE8_EVENT_REQ_FORCE_REBUILD", "0") == "1"
RUN_SUMMARY = os.environ.get("PHASE8_EVENT_REQ_RUN_SUMMARY", "1") == "1"

VARIANT = {
    "candidate_key": "event_request_plus",
    "feature_set": "cov_activity_tail_full_event_request",
}
...
def ensure_purchase_request_daily_features():
    path = os.path.join(PROJECT_ROOT, "data", "phase8a_prep", "purchase_request_daily_features.csv")
    if (not FORCE_REBUILD) and os.path.exists(path):
        return path
    t0 = time.time()
    rc = subprocess.run(
        [sys.executable, os.path.join(PROJECT_ROOT, "src", "analysis", "generate_phase8_purchase_request_prep_artifacts.py")],
        cwd=PROJECT_ROOT,
    ).returncode
    t1 = time.time()
    append_timing("", VARIANT["candidate_key"], "build_daily_features", "purchase_request_daily_features", t0, t1, "success" if rc == 0 else "failed")
    if rc != 0:
        raise RuntimeError("Failed to build purchase request daily features")
    return path


def ensure_event_shadow_assets(anchor_date):
    tag = anchor_tag(anchor_date)
    processed_dir = os.path.join(PROJECT_ROOT, "data", f"processed_v6_event_p8shadow_{tag}_v6_event")
    artifacts_dir = os.path.join(PROJECT_ROOT, "data", f"artifacts_v6_event_p8shadow_{tag}_v6_event")
    ...
    t0 = time.time()
    rc = subprocess.run(
        [sys.executable, os.path.join(PROJECT_ROOT, "src", "features", "build_features_v6_event_shadow_sku.py")],
        cwd=PROJECT_ROOT,
        env=env,
    ).returncode
    t1 = time.time()
    append_timing(anchor_date, "event_intent_plus", "build_event_shadow_assets", f"p8shadow_{tag}_v6_event", t0, t1, "success" if rc == 0 else "failed")
    if rc != 0:
        raise RuntimeError(f"Failed to build event shadow assets for {anchor_date}")
    return processed_dir, artifacts_dir


def ensure_event_request_assets(anchor_date):
    tag = anchor_tag(anchor_date)
    output_tag = f"p8eventreqshadow_{tag}_v6_event"
    ...
    ensure_event_shadow_assets(anchor_date)
    env = os.environ.copy()
    env["PYTHONIOENCODING"] = "utf-8"
    env["FEATURE_SPLIT_DATE"] = anchor_date
    env["FEATURE_VAL_MODE"] = VAL_MODE
    env["PHASE8_REQ_BASE_PREFIX"] = "p8shadow"
    env["PHASE8_REQ_OUTPUT_PREFIX"] = "p8eventreqshadow"
    env["PHASE8_REQ_KEEP_TAG"] = f"p7b_{tag}_v5_lite"
    t0 = time.time()
    rc = subprocess.run(
        [sys.executable, os.path.join(PROJECT_ROOT, "src", "features", "build_features_v6_purchase_request_shadow_sku.py")],
        cwd=PROJECT_ROOT,
        env=env,
    ).returncode
    t1 = time.time()
    append_timing(anchor_date, VARIANT["candidate_key"], "build_event_request_assets", output_tag, t0, t1, "success" if rc == 0 else "failed")
    if rc != 0:
        raise RuntimeError(f"Failed to build event+request shadow assets for {anchor_date}")
    return processed_dir, artifacts_dir


def run_training(anchor_date, processed_dir, artifacts_dir):
    base_exp = f"p8eventreq_{anchor_tag(anchor_date)}_{VARIANT['candidate_key']}_s{SEED}"
    ...
    t1 = time.time()
    append_timing(anchor_date, VARIANT["candidate_key"], "train", base_exp, t0, t1, "success" if rc == 0 else "failed")
    if rc != 0:
...
def main():
    ensure_dirs()
    init_timing_log()
    ensure_purchase_request_daily_features()
    print(f"[Phase8 event+request shadow] anchors={ANCHORS} seed={SEED}")
    for anchor_date in ANCHORS:
        processed_dir, artifacts_dir = ensure_event_request_assets(anchor_date)
        base_exp = run_training(anchor_date, processed_dir, artifacts_dir)
        run_eval(anchor_date, base_exp, processed_dir, artifacts_dir)

    if RUN_SUMMARY:
        subprocess.run(
            [sys.executable, os.path.join(PROJECT_ROOT, "src", "analysis", "summarize_phase8t_event_request_shadow_results.py")],
            cwd=PROJECT_ROOT,
            check=False,
        )
    print("[DONE] phase8 event+request shadow completed")
