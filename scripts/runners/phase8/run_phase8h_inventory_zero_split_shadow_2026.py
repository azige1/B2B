...
from run_phase8d_event_inventory_shadow_2026 import (
    ANCHORS,
    BASE_CFG,
    FORCE_REBUILD,
    SEED,
...
REPORTS_DIR = os.path.join(PROJECT_ROOT, "reports", "phase8_inventory_zero_split_shadow_2026")
MODEL_DIR = os.path.join(PROJECT_ROOT, "models_phase8_inventory_zero_split_shadow_2026")
TIMING_LOG = os.path.join(REPORTS_DIR, "phase8_inventory_zero_split_shadow_timing_log.csv")
...
def ensure_zero_split_assets(anchor_date):
    ...
    append_timing(
        anchor_date,
        ZERO_SPLIT_VARIANT["candidate_key"],
        "build_shadow_assets",
        output_tag,
        t0,
        t1,
    ...
    if rc != 0:
        raise RuntimeError(f"Failed to build inventory zero-split shadow assets for {anchor_date}")
    return processed_dir, artifacts_dir
...
def main():
    ensure_dirs()
    init_timing_log()
    print(f"[Phase8 inventory zero-split shadow 2026] anchors={ANCHORS} seed={SEED}")
    for anchor_date in ANCHORS:
        ensure_base_assets(anchor_date)
        zero_split_processed, zero_split_artifacts = ensure_zero_split_assets(anchor_date)
        zero_split_exp = run_training(anchor_date, ZERO_SPLIT_VARIANT, zero_split_processed, zero_split_artifacts)
        run_eval(anchor_date, ZERO_SPLIT_VARIANT, zero_split_exp, zero_split_processed, zero_split_artifacts)

    subprocess.run(
        [sys.executable, os.path.join(PROJECT_ROOT, "src", "analysis", "summarize_phase8h_inventory_zero_split_results.py")],
        cwd=PROJECT_ROOT,
        check=False,
    )
    print("[DONE] phase8 inventory zero-split shadow 2026 completed")
