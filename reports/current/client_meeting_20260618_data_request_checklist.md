...
| 确认项 | 说明 | 影响 |
| --- | --- | --- |
| `allow_external_api` | 商品图片是否允许发给外部多模态 API | 决定能否用最快的 API 打标方案 |
| `api_region_requirement` | 是否必须国内云、企业账号或私有化部署 | 决定合规和工程成本 |
| `image_sensitivity_level` | 图片是否包含未上市设计、样衣、敏感款式 | 决定是否需要脱敏或私有化 |
| `review_owner` | 设计/企划复核负责人 | 决定标签质量和人工成本 |
| `target_use_case` | 冷启动预测/企划复盘/防撞款/买手推盘 | 决定第一版标签 schema |
| `budget_range` | Demo、V1、V2 的可接受预算范围 | 决定是否只做 CSV，还是入库和 shadow 实验 |
