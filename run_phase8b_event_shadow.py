...
REPORTS_DIR = os.path.join(PROJECT_ROOT, "reports", "phase8_event_shadow")
MODEL_DIR = os.path.join(PROJECT_ROOT, "models_phase8_event_shadow")
TIMING_LOG = os.path.join(REPORTS_DIR, "phase8_event_shadow_timing_log.csv")

ANCHORS = ["2025-10-01", "2025-11-01", "2025-12-01"]
SEED = "2028"
TREE_BACKEND = "lightgbm"
VAL_MODE = "single_anchor"
FORCE_REBUILD = os.environ.get("PHASE8_EVENT_FORCE_REBUILD", "0") == "1"
RUN_SUMMARY = os.environ.get("PHASE8_EVENT_RUN_SUMMARY", "1") == "1"
...
def ensure_shadow_assets(anchor_date):
    ...
    t1 = time.time()
    append_timing(anchor_date, VARIANT["candidate_key"], "build_shadow_assets", output_tag, t0, t1, "success" if rc == 0 else "failed")
    if rc != 0:
        raise RuntimeError(f"Failed to build shadow assets for {anchor_date}")
    return processed_dir, artifacts_dir


def run_training(anchor_date, processed_dir, artifacts_dir):
    base_exp = f"p8event_{anchor_tag(anchor_date)}_{VARIANT['candidate_key']}_s{SEED}"
...
def main():
    ensure_dirs()
    init_timing_log()
    print(f"[Phase8 event shadow] anchors={ANCHORS} seed={SEED}")
    for anchor_date in ANCHORS:
        processed_dir, artifacts_dir = ensure_shadow_assets(anchor_date)
        base_exp = run_training(anchor_date, processed_dir, artifacts_dir)
        run_eval(anchor_date, base_exp, processed_dir, artifacts_dir)

    if RUN_SUMMARY:
        subprocess.run(
            [sys.executable, os.path.join(PROJECT_ROOT, "src", "analysis", "summarize_phase8b_event_shadow_results.py")],
            cwd=PROJECT_ROOT,
            check=False,
        )
    print("[DONE] phase8 event shadow completed")
